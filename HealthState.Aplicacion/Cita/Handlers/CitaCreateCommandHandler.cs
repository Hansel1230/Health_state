...
{
    public class CitaCreateCommandHandler(IUnitOfWork unitOfWork, IMapper mapper) : IRequestHandler<CitaCreateCommand, CitaModel>
    {
        public async Task<CitaModel> Handle(CitaCreateCommand request, CancellationToken cancellationToken)
        {
            ...
            && x.Medico.Cedula == request.MedicoCedula && x.Paciente.Cedula == request.PacienteCedula))
            ...
            var entityCita = mapper.Map<HealthState.Dominio.Cita>(request);

            entityCita.PacienteId = entityPaciente.PacienteId;
            entityCita.MedicoId = entityMedico.MedicoId;

            await repositoryCita.InsertAsync(entityCita);
