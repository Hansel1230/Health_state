...
{
    public class DetalleFacturaUpdateCommandHandler(IUnitOfWork unitOfWork, IMapper mapper) : IRequestHandler<DetalleFacturaUpdateCommand, DetalleFacturaModel>
    {
        public async Task<DetalleFacturaModel> Handle(DetalleFacturaUpdateCommand request, CancellationToken cancellationToken)
        {
            ...
            var entityDetalle = await repositoryDetalle.FirstAsync(x => x.DetalleId == request.DetalleId);

            mapper.Map(request, entityDetalle);
